...
function solution(a, b, n) {    //=> b가 사용이 안됨 확인필요**
    var answer = 0;
    ...
    while(loof){
        console.log('s',temp,a)
        temp%a == 0 ? cola.push((temp/a)*b) : cola.push(Math.floor(temp/a)*b);
        answer+= (temp%a == 0 ? (temp/a)*b : Math.floor(temp/a)*b);
        console.log(Math.floor(temp/a) + temp%a);
        temp = (Math.floor(temp/a)*b)+temp%a;   //-> 문제에서 콜라병수를 구할때 늘어나는 예시가 없었음
                                                //   몫에 얻는 병수를 곱해줘야하는 예시가 없어서 한참 헤맸음 

        if(temp < a) loof = false;
    }
