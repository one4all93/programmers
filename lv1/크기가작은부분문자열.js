...
function solution(t, p) {
    var answer = 0;
    let length = p.length;
    // console.log(t,p , p.length);
    // let temp = [];
    let cnt = 0;
    for(let i=0; i<t.length; i++){
        // console.log(i , length+i); // -> slice할때 시작위치
        // console.log(i,t.slice(i,length+i).length);
        if(length == t.slice(i,length+i).length){
            // temp[i] = t.slice(i,length+i);
            t.slice(i,length+i) <= p ? cnt++ : undefined;
        }
    }
