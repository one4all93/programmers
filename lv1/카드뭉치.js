...
function solution(cards1, cards2, goal) {
    ...
    // goal원본 배열에 지장을 안주기위해서 얕은복사로 배열복사
    let temp = [...goal];

    // goal배열 반복문 돌리면서 하나씩 비교
    goal.forEach(word=>{
        // console.log(word);
        // 무조건 앞에 있는 값부터 비교를 진행해야해서 0번 인덱스랑 shift()를 사용함**
        // card1의 첫번째 값&temp첫번째 값 == word랑 같으면 temp와cards1 첫번째 값 삭제
        if(cards1[0] == word && temp[0] == word){
            // console.log('cards1',word , temp);
            cards1.shift();
            temp.shift();
        // card2의 첫번째 값&temp첫번째 값 == word랑 같으면 temp와cards2 첫번째 값 삭제
        }else if(cards2[0] == word && temp[0] == word){
            // console.log('cards2',word , temp);
            cards2.shift();
            temp.shift();
        }
    })
    ...
    // temp의 값이 값이 남아있나/없나 확인
    answer = temp.length ==0 ? "Yes" : "No";
    console.log(answer);
    return answer;
}