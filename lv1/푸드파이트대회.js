...
function solution(food) {
    ...
    // 주어진 음식갯수배열 반복문 돌림(마지막에 0을 넣어주기위해서 <=로 조건 설정)
    for(let i=0; i<=food.length; i++){
        // 0번은 물이니까 제외
        if(i!=0 && i < food.length){
            // 주어진 음식갯수 2로 나눈갯수만큼 반복해서 i를 음식배치배열에 넣어줌
            for(let j=1; j<=Math.floor(food[i]/2); j++){
                place.push(i);
            }
        // 마지막에 0을 넣어주기 위해서 주어진 배열 크기와 같을때 == 마지막에 0을 넣어줌
        }else if(i == food.length){
            place.push(0);
        }
    }

    // 음식배치한 배열을 얕은복사로 복사
    let temp = [...place];  // 1223330

    // 기존 음식배치배열 거꾸로 하고 거꾸로된 배열에서 첫번째값 0 삭제
    temp.reverse().shift(); // 0333221 -> 333221
    // 
    place.push(...temp);    // 1223330 + 3332211
    answer = place.join(''); // [1, 2, 2, 3, 3, 3, 0, 3, 3, 3, 2, 2, 1] -> 문자로 합침
    console.log(answer);
